...
real_XMAX=140;  %图片宽度（pixel）
real_YMAX=140;  %图片高度（pixel）
  
%细胞基础参数 %   
LAM_AREA=2;                       %面积模量
LAM_PERIMETER=0.4;                %周长模量
TARGET_CELL_SIZE=250;             %细胞面积(/pixel)
TARGET_PERIMETER_SIZE= 180;       %细胞周长(8连通域下的周长)
J_LL=20;       %细胞间黏附能（无用）
J_LM=20;       %细胞-ECM黏附能
...
persist_time=5;     %伪足小体寿命(MCS)
PEG_act=0;          %PEG处新生骨架的活性
PODO_act=45;        %PEG处有伪足小体激发的细胞骨架的活性，须大于MAX_ACT
P_Grow0=0.05*1;       %伪足小体生成概率
   threshold_podo=0.75; %生成伪足小体的act活性阈值（threshold_podo*MAX_ACT）
...
% 边界条件限制 %
Distance=30;  %条纹间距（pixel）

N_cir=2;   %圆排数（不改）
R_cir=15;  %圆半径（pixel）
Distance=Distance+R_cir*2;
h_Distance=Distance*sqrt(3);
...
sigma_order=find(((X-center_xc).^2+(Y-center_yc).^2)<(R_cir-1)^2);
order=randi(length(sigma_order),[N_cell,1]);
sigma(sigma_order(order))=1:N_cell;

Substrate_label=zeros(XMAX,YMAX);
sigma_orderS={};
for i=1:N_cir
    for j=1:N_cir
         sigma_orderS{i*2+j-2}=find(((X-center_xc-(i-3/2)*Distance).^2+(Y-center_yc-(j-3/2)*h_Distance).^2)<R_cir^2);
         Substrate_label(sigma_orderS{i*2+j-2})=2*(i-1)+j;
    end
end
for i=5:7
    sigma_orderS{i}=find(((X-center_xc-(i-6)*Distance).^2+(Y-center_yc).^2)<R_cir^2);
